...
import { ActivityFormProps, ActivityFormData } from './types';
...
const ActivityForm: React.FC<ActivityFormProps> = ({ activity, onSubmit, onCancel, loading = false }) => {
  const [initialValues, setInitialValues] = useState<Partial<ActivityFormData>>({
    id: '',
    activity_name: '',
    description: '',
    start_date: null,
  ...
  useEffect(() => {
    if (activity) {
      const processedActivity = {
        ...activity,
        start_date: activity.start_date ? parseISO(activity.start_date as string) : null,
        end_date: activity.end_date ? parseISO(activity.end_date as string) : null,
        registration_deadline: activity.registration_deadline 
          ? parseISO(activity.registration_deadline as string) 
          : null,
        activity_schedules: activity.activity_schedules?.length > 0
          ? activity.activity_schedules.map(schedule => ({
              ...schedule,
              start_time: schedule.start_time ? parseISO(schedule.start_time as string) : null,
              end_time: schedule.end_time ? parseISO(schedule.end_time as string) : null,
            }))
          : [{
              start_time: null,
              end_time: null,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setFieldError }) => {
      try {
        ...
        await onSubmit(formattedValues as ActivityFormData);
      } catch (error: any) {
  ...
  const handleScheduleChange = useCallback((index: number, field: string, value: any) => {
  ...
  const removeSchedule = useCallback((index: number) => {
    if (formik.values.activity_schedules.length > 1) {
      const newSchedules = formik.values.activity_schedules.filter((_: any, i: number) => i !== index);
      formik.setFieldValue('activity_schedules', newSchedules);
    }
  }, [formik]);
          ...
          <Grid item xs={12}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <FormField
                    ...
                    label="Start Date"
                    type="datetime-local"
                    value={formik.values.start_date ? format(formik.values.start_date, "yyyy-MM-dd'T'HH:mm") : ''}
                    onChange={formik.handleChange}
                    InputLabelProps={{
                      shrink: true,
                    }}
                ...
                <Grid item xs={12} md={6}>
                  <FormField
                    ...
                    label="End Date"
                    type="datetime-local"
                    value={formik.values.end_date ? format(formik.values.end_date, "yyyy-MM-dd'T'HH:mm") : ''}
                    onChange={formik.handleChange}
                    InputLabelProps={{
                      shrink: true,
                    }}
            ...
            <FormSection>
              <FormField
                ...
                type="datetime-local"
                value={formik.values.registration_deadline ? format(formik.values.registration_deadline, "yyyy-MM-dd'T'HH:mm") : ''}
                onChange={formik.handleChange}
                InputLabelProps={{
                  shrink: true,
                }}
