...
import { Activity, ActivityCategory, ActivityStatus } from "@/types/entities";
...
const MyContributorActivity = () => {
  ...
  const fetchActivities = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/activities/my-contributor?${queryString}`,
        {
          method: "GET",
          headers: {
  ...
  const handleFilterChange = (field: 'status' | 'category') => (event: SelectChangeEvent<string>) => {
    setFilters((prev) => ({
      ...prev,
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 3, display: "flex", gap: 2 }}>
        ...
        <FormControl sx={{ minWidth: 200 }}>
          <Select
            value={sorting.field}
            onChange={(e: SelectChangeEvent<SortField>) => handleSortChange(e.target.value as SortField)}
            displayEmpty
          >
